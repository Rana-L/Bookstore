  ...
  document.getElementById("b").addEventListener("click", (e) => {
    ...
    fetch(url, {
      ...
      .then((resJ) => {
       ...
       const lastFourDigits = c.slice(-4);
       location.href = `success.html?lastFourDigits=${lastFourDigits}`;

    
      });
  });

  // Function to display the last four digits dynamically
  function displayLastFourDigits(cardNumber) {
    // Update the HTML element to display the last four digits
    document.getElementById('lastFourDigits').textContent = cardNumber.slice(-4);
  }

  // Listen for input events on the card number input field
  document.getElementById('cardNumberInput').addEventListener('input', function() {
    // Retrieve the value entered by the user
    var cardNumber = this.value;

    // Display the last four digits dynamically
    displayLastFourDigits(cardNumber);
  });
